...
class MedicalManager(Medical):
    def medicalinfo(self, request):
        ...
        return render_template("Admin/Info/medicalinfo.html", medicalinfo=medicalinfo)
    ...
    def updateMedicalInfo(self, request):
        if request.method == "POST":
            ...
            self.resident_updater.update_medicalinfo(updatemedicalid, updatebmiclassification, updatemedicalcondition,
                                                     updatecovidvaccinated, updatemaintenancemedicine,
                                                     updatephysicalfitness)
            return redirect("/admin/medicalinfo")
        ...
        return render_template("Admin/Update/updateMedical.html", editmedicalid=editmedicalid,
                               editbmiclassification=editbmiclassification,
                               editmedicalcondition=editmedicalcondition,
                               editcovidvaccinated=editcovidvaccinated,
                               editmaintenancemedicine=editmaintenancemedicine,
