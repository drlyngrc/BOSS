...
class DerivedTables:
    ...
    def householdinfo(self, request):
        ...
        if request.method == "POST":
            if "action" in request.form and request.form["action"] == "submit":
                ...
                return render_template("Info/householdinfo.html", householdinfo=householdinfo)

        return render_template("Info/householdinfo.html", householdinfo=[])
    ...
    def incomerecord(self, request):
        ...
        return render_template("Info/incomerecord.html", incomerecord=incomerecord)

    def unemploymentinfo(self):
        unemploymentinfo = self.read_instance.read_unemploymentinfo()
        return render_template("Info/unemploymentinfo.html", unemploymentinfo=unemploymentinfo)

    def scholarshipinfo(self):
        scholarshipinfo = self.read_instance.read_scholarshipinfo()
        return render_template("Info/scholarshipinfo.html", scholarshipinfo=scholarshipinfo)
